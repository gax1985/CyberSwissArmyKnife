---
title: "Hack The Box Academy - Footprinting - FTP"
...
```
TheCloudsAbove@htb[/htb]$ sudo apt install vsftpd
```
...
```
TheCloudsAbove@htb[/htb]$ cat /etc/vsftpd.conf | grep -v "#"
```
...
```
TheCloudsAbove@htb[/htb]$ cat /etc/ftpusers
...
```
TheCloudsAbove@htb[/htb]$ ftp 10.129.14.136
...
```
TheCloudsAbove@htb[/htb]$ ls | grep Notes.txt
...
```
TheCloudsAbove@htb[/htb]$ wget -m --no-passive ftp://anonymous:anonymous@10.129.14.136
...
```
TheCloudsAbove@htb[/htb]$ touch testupload.txt
```
...
```
TheCloudsAbove@htb[/htb]$ sudo nmap --script-updatedb
...
```
TheCloudsAbove@htb[/htb]$ find / -type f -name ftp* 2>/dev/null | grep scripts
...
```
TheCloudsAbove@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136
...
```
TheCloudsAbove@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136 --script-trace
...
```
TheCloudsAbove@htb[/htb]$ nc -nv 10.129.14.136 21
```

FTP

```
TheCloudsAbove@htb[/htb]$ telnet 10.129.14.136 21
```
...
```
TheCloudsAbove@htb[/htb]$ openssl s_client -connect 10.129.14.136:21 -starttls ftp
...
FTP is a way to **send and receive files** between computers over the internet.

### How it works:

- It uses **two connections**:
    - **Control channel (Port 21)**: For sending commands like “upload this file” or “download that file.”
    - **Data channel (Port 20)**: For actually transferring the files.

### Modes:

- **Active Mode**: The server tries to connect back to the client. This can be blocked by firewalls.
- **Passive Mode**: The client connects to the server for both control and data. This works better when firewalls are involved.

### Security:

- FTP sends data in **plain text**, including usernames and passwords. This means attackers can **sniff** the data if the network isn’t secure.
- Some servers allow **anonymous access**, meaning anyone can download or upload files without logging in. This is risky, so access is usually limited.
...
- Uses **UDP** instead of TCP → less reliable.
- **No login or password** → no user authentication.
- Only works with files that are **shared for everyone**.
- Used mostly in **local networks**, like for routers or firmware updates.

### Basic Commands:

- `connect`: Set the server you want to talk to.
- `get`: Download a file.
- `put`: Upload a file.
- `quit`: Exit the program.
- `status`: Show current settings.
- `verbose`: Show more details during transfers.

---

## ⚙️ **vsFTPd (Very Secure FTP Daemon)**

This is a popular FTP server used on Linux systems.

### Configuration file:

- Located at: `/etc/vsftpd.conf`
- You can see active settings using: cat /etc/vsftpd.conf | grep -v "#"

### Important settings:

- `anonymous_enable=NO`: Disables anonymous access.
- `local_enable=YES`: Allows local users to log in.
- `ssl_enable=NO`: SSL is off (so no encryption).
- `xferlog_enable=YES`: Logs file transfers.

### `/etc/ftpusers` file:

- Lists users who **cannot** use FTP.
- Example: guest john kevin

---

## 🧠 Summary for Cybersecurity Students

- **FTP is useful but insecure** unless encrypted (e.g., FTPS or SFTP).
- **TFTP is even less secure**, so only use it in trusted environments.
- **vsFTPd** is a good tool to learn how FTP servers are configured and secured.
- Always think about **firewalls, encryption, and user access** when dealing with file transfer protocols.

---
## 🧨 Dangerous FTP Settings (Explained Simply)

These settings in an FTP server can make it **less secure**, especially if you're testing or working in a real-world network.

### 🔓 **Anonymous Access**

This means **anyone** can log in without a username or password.

Here are some settings that control anonymous access:

|Setting|What it does|
|---|---|
|`anonymous_enable=YES`|Allows anyone to log in as "anonymous".|
|`anon_upload_enable=YES`|Lets anonymous users upload files.|
|`anon_mkdir_write_enable=YES`|Lets them create folders.|
|`no_anon_password=YES`|Doesn’t ask for a password.|
|`anon_root=/home/username/ftp`|Sets the folder they can access.|
|`write_enable=YES`|Allows file changes like delete, rename, upload, etc.|

> 🔐 **Why it’s risky**: If these are enabled, attackers might upload malware or steal files.

---

## 🖥️ **Connecting to an FTP Server**

When you connect using the `ftp` command, you might see something like:
...
- `220` is a **status code** meaning the server is ready.
- `230` means **login was successful**.
- You can then use commands like `ls` to **list files**.

---

## 🔍 **Useful FTP Commands for Info Gathering**

These commands help you learn more about the server:

|Command|What it does|
|---|---|
|`status`|Shows current connection settings.|
|`debug`|Turns on detailed output (good for troubleshooting).|
|`trace`|Shows the actual packets being sent.|

---

## 🛡️ **Other Security-Related Settings**

|Setting|What it does|
|---|---|
|`dirmessage_enable=YES`|Shows a message when entering a folder.|
|`chown_uploads=YES`|Changes ownership of uploaded files.|
|`chown_username=username`|Sets who owns uploaded files.|
|`local_enable=YES`|Allows local system users to log in.|
|`chroot_local_user=YES`|Locks users into their home folder.|
|`chroot_list_enable=YES`|Uses a list of users to apply the lock.|
|`hide_ids=YES`|Hides real usernames and groups in file listings.|
|`ls_recurse_enable=YES`|Allows listing folders and subfolders recursively.|

> 🧠 **Why it matters**: These settings affect how much info you can see and what actions users can take. As a cybersecurity student, you’ll often check these to find weaknesses.

---

## 🧪 Summary for You

- **Anonymous FTP** is useful for sharing files but can be dangerous if not restricted.
- **vsFTPd** has many settings that control access, visibility, and file permissions.
- Use commands like `status`, `debug`, and `trace` to gather more info during an assessment.
- Always think about **what an attacker could do** if these settings are misconfigured.

---

## 🕵️‍♂️ Hiding IDs (`hide_ids=YES`)

This setting makes **all files and folders** on the FTP server show the owner as `ftp` instead of the real usernames.

### 🔒 Why is this important?

- Normally, files show which user created or owns them.
- If attackers see real usernames, they might try **brute-force attacks** on services like FTP or SSH.
- By hiding IDs, the server protects user info and makes it harder to guess valid usernames.

> 🧠 Think of it like hiding name tags on lockers so no one knows whose stuff is inside.

---

## 📂 Recursive Listing (`ls_recurse_enable=YES`)

This setting lets you use the `ls -R` command to **see all folders and files**, including subfolders, in one go.

### 🔍 Why is this useful?

- You get a **full map** of the FTP server’s file structure.
- Helps you find **interesting files** quickly, like `.txt`, `.docx`, `.pptx`, etc.
- You might discover sensitive documents or folders like `Employees`, `Clients`, or `Documents`.

> 🧠 It’s like opening all drawers in a cabinet at once instead of checking one by one.

---

## 📥 Uploading & Downloading Files

If the server allows uploads and downloads:

- You can **download files** to inspect them for sensitive info.
- You can **upload files**—this is risky if attackers upload malicious scripts.

### ⚠️ Security Risk:

- If the server has a vulnerability like **LFI (Local File Inclusion)**, attackers might upload a file that gets executed by the system.
- This can lead to **Remote Command Execution (RCE)**—basically, running commands on the server remotely.

> 🧠 Imagine uploading a file that tricks the server into running a command like `rm -rf /`—very dangerous!
...
|---|---|---|
|`hide_ids=YES`|Hides real usernames|Prevents brute-force attacks|
|`ls_recurse_enable=YES`|Lists all folders/files|Helps find sensitive data|
|Upload/Download|Transfer files|Can be used for attacks like LFI or RCE|

---

Would you like help practicing FTP enumeration in a lab, or want me to explain how to detect and exploit LFI or RCE vulnerabilities using FTP?